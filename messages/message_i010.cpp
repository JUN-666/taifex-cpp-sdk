...
bool parse_i010_body(const unsigned char* body_data,
    ...
    std::span<const unsigned char> ref_price_span(body_data + offset, REF_PRICE_BCD_LEN);
    std::string ref_price_str = bcdBytesToAsciiStringHelper(std::as_bytes(ref_price_span), REF_PRICE_DIGITS);
    ...
    std::span<const unsigned char> dec_loc_span(body_data + offset, DEC_LOC_BCD_LEN);
    std::string dec_loc_str = bcdBytesToAsciiStringHelper(std::as_bytes(dec_loc_span), DEC_LOC_DIGITS);
    ...
    std::span<const unsigned char> strike_dec_loc_span(body_data + offset, STRIKE_DEC_LOC_BCD_LEN);
    std::string strike_dec_loc_str = bcdBytesToAsciiStringHelper(std::as_bytes(strike_dec_loc_span), STRIKE_DEC_LOC_DIGITS);
    ...
    std::span<const unsigned char> begin_date_span(body_data + offset, BEGIN_DATE_BCD_LEN);
    out_msg.begin_date = bcdBytesToAsciiStringHelper(std::as_bytes(begin_date_span), BEGIN_DATE_DIGITS);
    ...
    std::span<const unsigned char> end_date_span(body_data + offset, END_DATE_BCD_LEN);
    out_msg.end_date = bcdBytesToAsciiStringHelper(std::as_bytes(end_date_span), END_DATE_DIGITS);
    ...
    std::span<const unsigned char> flow_group_span(body_data + offset, FLOW_GROUP_BCD_LEN);
    std::string flow_group_str = bcdBytesToAsciiStringHelper(std::as_bytes(flow_group_span), FLOW_GROUP_DIGITS);
    ...
    std::span<const unsigned char> delivery_date_span(body_data + offset, DELIVERY_DATE_BCD_LEN);
    out_msg.delivery_date = bcdBytesToAsciiStringHelper(std::as_bytes(delivery_date_span), DELIVERY_DATE_DIGITS);
