#include "message_i083.h"
#include "common_header.h" // For CoreUtils::packBcdToAscii (potentially, if CommonHeader itself uses it - though not directly by this parser)
...
bool parse_i083_body(const unsigned char* body_data,
                     ...
                     MessageI083& out_msg) {
    if (!body_data) {
        // std::cerr << "Error: body_data is null for I083." << std::endl;
        return false;
    }

    size_t offset = 0;

    // Minimum length: PROD-ID (20) + PROD-MSG-SEQ (5) + CALCULATED-FLAG (1) + NO-MD-ENTRIES (1) = 27 bytes
    const size_t MIN_FIXED_PART_LEN = 27;
    if (body_length < MIN_FIXED_PART_LEN) {
        // std::cerr << "Error: body_length (" << body_length
        //           << ") is too short for I083 fixed part. Expected at least "
    ...
    // 3. CALCULATED-FLAG: X(1), Length 1
    const size_t CALC_FLAG_LEN = 1;
    out_msg.calculated_flag = static_cast<char>(body_data[offset]);
    offset += CALC_FLAG_LEN;

    // 4. NO-MD-ENTRIES: 9(2), Length 1
    ...
    // Each entry: MD-ENTRY-TYPE (1) + SIGN (1) + MD-ENTRY-PX (5) + MD-ENTRY-SIZE (4) + MD-PRICE-LEVEL (1) = 12 bytes
    const size_t MD_ENTRY_SIZE_BYTES = 12;
