...
TaifexSdk::TaifexSdk() : initialized_(false) {
    ...
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "TaifexSdk instance created.");
}

TaifexSdk::~TaifexSdk() {
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "TaifexSdk instance destroyed.");
...
bool TaifexSdk::initialize(/* Potential config parameters */) {
    ...
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "TaifexSdk initializing...");
    // Perform any other one-time setup for the SDK.

    initialized_ = true;
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "TaifexSdk initialized successfully.");
    return true;
}
...
void TaifexSdk::process_message(const unsigned char* raw_message, size_t length) {
    if (!initialized_) {
        CoreUtils::Logger::Log(CoreUtils::LogLevel::WARNING, "TaifexSdk::process_message called before initialization.");
        return;
    }
    if (!raw_message || length == 0) {
        CoreUtils::Logger::Log(CoreUtils::LogLevel::WARNING, "TaifexSdk::process_message called with null or empty message.");
        return;
    }
    ...
    if (length < CoreUtils::CommonHeader::HEADER_SIZE + 1 + 2) { // Min length for header, checksum, term_code
        CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Message too short for even basic validation.");
        return;
    }
    ...
    if (!CoreUtils::CommonHeader::parse(raw_message, length, temp_header)) {
         CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Initial header parse for validation failed (message too short for header).");
        return;
    }
    ...
    if (length != expected_total_length) {
        CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Message length mismatch. Expected: " +
    ...
    CoreUtils::Logger::Log(CoreUtils::LogLevel::DEBUG, "Checksum validation passed.");
    ...
    CoreUtils::Logger::Log(CoreUtils::LogLevel::DEBUG, "CommonHeader parsed. BodyLength: " + std::to_string(header.getBodyLength()) +
    ...
    CoreUtils::MessageType msg_type = CoreUtils::MessageIdentifier::identify(header);
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Identified MessageType: " + CoreUtils::messageTypeToString(msg_type));
    ...
    std::string product_id_for_handler;
    if (msg_type == CoreUtils::MessageType::I010_PRODUCT_BASIC_DATA) {
        // PROD-ID-S is X(10) at offset 0 of I010 body
        if (body_length >= 10) {
            product_id_for_handler.assign(reinterpret_cast<const char*>(body_ptr), 10);
        }
    } else if (msg_type == CoreUtils::MessageType::I081_ORDER_BOOK_UPDATE || msg_type == CoreUtils::MessageType::I083_ORDER_BOOK_SNAPSHOT) {
    ...
    dispatch_message_body(body_ptr, body_length, msg_type, header, product_id_for_handler);
}
...
// ... (Constructor, Destructor, initialize, process_message, getters, dispatch_message_body remain) ...
...
std::string get_base_prod_id_for_i010_lookup(const std::string& prod_id_from_message) {
    ...
    if (prod_id_from_message.length() > 10) {
        ...
        CoreUtils::trim(potential_base); // Assuming CoreUtils::trim exists
...
void TaifexSdk::handle_i010(const unsigned char* body_ptr, uint16_t body_len, const CoreUtils::CommonHeader& header) {
    ...
    if (SpecificMessageParsers::parse_i010_body(body_ptr, body_len, i010_msg)) {
        ...
        CoreUtils::trim(prod_id_s); // Ensure key is clean

        CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Parsed I010 for PROD-ID-S: " + prod_id_s +
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Failed to parse I010 body.");
    }
}

OrderBookManagement::OrderBook* TaifexSdk::get_or_create_order_book(const std::string& product_id_from_message_body) {
    ...
    std::string base_prod_id_for_i010 = get_base_prod_id_for_i010_lookup(product_id_from_message_body);
    ...
    if (it_info != product_info_cache_.end()) {
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Creating new OrderBook for PROD-ID: " + product_id_from_message_body +
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::WARNING, "No I010 product info found for PROD-ID-S: " +
...
void TaifexSdk::handle_i081(const unsigned char* body_ptr, uint16_t body_len, const CoreUtils::CommonHeader& header, const std::string& /*temp_product_id_from_dispatch*/) {
    ...
    if (SpecificMessageParsers::parse_i081_body(body_ptr, body_len, i081_msg)) {
        ...
        CoreUtils::trim(current_prod_id);

        CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Parsed I081 for PROD-ID: " + current_prod_id +
        ...
        if (ob) {
            ...
            CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Failed to get/create OrderBook for PROD-ID: " + current_prod_id + " for I081. Message unprocessed.");
            // TODO: Strategy for messages for products without I010? Queue them? Discard?
        }
    } else {
        CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Failed to parse I081 body.");
    }
}

void TaifexSdk::handle_i083(const unsigned char* body_ptr, uint16_t body_len, const CoreUtils::CommonHeader& header, const std::string& /*temp_product_id_from_dispatch*/) {
    ...
    if (SpecificMessageParsers::parse_i083_body(body_ptr, body_len, i083_msg)) {
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Parsed I083 for PROD-ID: " + current_prod_id +
        ...
        if (ob) {
            ...
            CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Failed to get/create OrderBook for PROD-ID: " + current_prod_id + " for I083. Message unprocessed.");
        }
    } else {
        CoreUtils::Logger::Log(CoreUtils::LogLevel::ERROR, "Failed to parse I083 body.");
    }
}

void TaifexSdk::handle_i001(const CoreUtils::CommonHeader& header) {
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Processing Heartbeat I001. Channel: " + std::to_string(header.getChannelId()) + ", Seq: " + std::to_string(header.getChannelSeq()));
    // No specific state change other than sequence number already handled by is_sequence_valid
}

void TaifexSdk::handle_i002(const CoreUtils::CommonHeader& header) {
    // I002: "若該CHANNEL屬即時行情群組則須清空各商品委託簿,並重置該傳輸群組之群組序號,同時重置各商品行情訊息流水序號"
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Processing Sequence Reset I002 for Channel: " + std::to_string(header.getChannelId()));
    ...
    for (auto& pair_ob : order_books_) { // C++11: auto& pair_ob : order_books_
        CoreUtils::Logger::Log(CoreUtils::LogLevel::DEBUG, "Resetting OrderBook for PROD-ID: " + pair_ob.first + " due to I002.");
    ...
    CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "Channel sequence for Channel " + std::to_string(channel_id) + " reset.");

    // "同時重置各商品行情訊息流水序號" - this is handled by OrderBook::reset() which sets its last_prod_msg_seq_ to 0.
}

bool TaifexSdk::is_sequence_valid(const CoreUtils::CommonHeader& header) {
    uint32_t channel_id = header.getChannelId();
    ...
    if (it == channel_sequences_.end()) {
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::INFO, "First message for Channel " + std::to_string(channel_id) +
    ...
    if (current_channel_seq == last_known_seq + 1) {
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::WARNING, "Out-of-order/replay Channel Seq for Channel " + std::to_string(channel_id) +
        ...
        CoreUtils::Logger::Log(CoreUtils::LogLevel::WARNING, "Gap detected in Channel Seq for Channel " + std::to_string(channel_id) +
